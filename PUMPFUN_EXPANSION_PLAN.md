# طرح جامع گسترش و معماری نهایی: ادغام امکانات PumpFun در پروژه Tambr (ریال‌توکن)

این سند، طرح جامع و معماری نهایی برای گسترش پروژه **Tambr (ریال‌توکن)** است تا تمام ویژگی‌های کلیدی پلتفرم **PumpFun** را در خود جای دهد، در حالی که ساختار Monorepo موجود و ویژگی‌های پیشرفته مانند KYC/AML و Social Recovery Wallet (SRW) حفظ شوند. هدف، تبدیل Tambr به یک پلتفرم کامل، بی‌نقص و آماده تولید برای راه‌اندازی توکن‌های جدید با پشتوانه ریالی و منطق DBC است.

## ۱. نقشه ویژگی‌ها: PumpFun در مقابل Tambr

پروژه Tambr در حال حاضر دارای یک هسته مالی قوی (DBC، Stablecoin) و ویژگی‌های سازمانی (KYC، SRW، Gasless) است. ادغام PumpFun نیازمند تغییر تمرکز DBC از یک منحنی قیمت‌گذاری ثابت به یک مکانیزم کشف قیمت پویا و خودکار است.

| ویژگی PumpFun | هدف اصلی | پیاده‌سازی در Tambr | جزء معماری |
| :--- | :--- | :--- | :--- |
...
هسته اصلی این گسترش، بازنویسی منطق **Dynamic Bonding Curve (DBC)** است تا از مدل ذخایر مجازی PumpFun استفاده کند.
...
برای پشتیبانی از مکانیزم ALP و تجربه کاربری PumpFun، سرویس‌های بک‌اند نیاز به گسترش دارند.
...
رابط کاربری باید سادگی و سرعت PumpFun را منعکس کند، در حالی که ویژگی‌های پیشرفته Tambr را به صورت یکپارچه ارائه دهد.
